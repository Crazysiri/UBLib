...
@implementation LTMediaLibraryTool
/**
...
+ (void)saveVideoWithVideoUrl:(NSURL *)videoUrl assetCollectionName:(NSString *)assetCollectionName completion:(SaveVideoCompletionBlock)saveVideoCompletionBlock
{
    ...
    if (![fm fileExistsAtPath:videoUrl.path]) {
        ...
        {
            NSString *des = [NSString stringWithFormat:@"文件不存在： %@",videoUrl.path];
            NSError *error = [NSError errorWithDomain:@"文件不存在" code:-1 userInfo:@{NSLocalizedDescriptionKey:des}];
